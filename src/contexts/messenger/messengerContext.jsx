...
export function MessengerProvider({ children }) {
  ...
  const selectChat = React.useCallback(
    (id) => {
      ...
      if (id[0] === "-") {
        ...
        setSelectedChat(
          state.chats.find((stateChat) => stateChat.id === chat?.id)
        );
      }
    },
    [state.chats]
  ...
  const search = React.useCallback(
    (searchInputValue, usersOnly = false) => {
      if (!searchInputValue) {
        dispatch({
  ...
  return (
    <MessengerContext.Provider value={v}>{children}</MessengerContext.Provider>
  );
}
